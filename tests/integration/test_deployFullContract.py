...
def testContract(consentManager):
    
    for i in range(0,5):
        print("account" + " " + str(i) + " " + "is" + " " + str(accounts[i]))
    ...
    print("Hospital Role assigned to accounts[1].")
    ...
    print("Approver role assigned to accounts[2].")
    
    consentManager.applyAsConsumer(1, {"from": accounts[3]})
    assert consentManager.returnAppliedConsumers(0, {"from": accounts[0]}) == accounts[3]
    print("accounts[3] applied as consumer, to be approved by approver.")

    consentManager.consumerApproval(accounts[3], 1, {'from': accounts[2]})
    assert consentManager.returnConsumerMappingBool(accounts[3])
    print("accounts[3] has been approved.")
